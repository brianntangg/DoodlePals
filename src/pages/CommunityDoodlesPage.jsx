...
function CommunityDoodlesPage() {
    ...
    const fetchDoodles = async (reset = true) => {
        ...
        try {
            const allDoodles = await db.getAllCommunityDoodles();
            ...
            const processedDoodles = allDoodles.map(doodle => {
                ...
                return {
                    ...
                    likes: Array.isArray(doodle.likes) ? doodle.likes : [],
                };
            }).filter(Boolean);
            ...
            sorted.sort((a, b) => {
                switch (sortBy) {
                    ...
                    case "mostActive":
                        { const aActivity = (a.likes?.length || 0) + Object.keys(a.comments || {}).length;
                        const bActivity = (b.likes?.length || 0) + Object.keys(b.comments || {}).length;
    ...
    const shouldShowLoadMore = !loading && !loadingMore && filteredDoodles.length > visibleCount;

    return (
        <Box p={10}>
            <Heading size="lg" mb={6}>Community Doodles</Heading>

            <HStack spacing={4} mb={8}>
                <Box>
                    <Text fontSize="sm" mb={1}>Filter by prompt:</Text>
                    <Select
            ...
            {loading ? (
                ...
                <Center py={12}>
                    <Box textAlign="center">
                        <Heading size="md" mb={2}>No doodles found</Heading>
                        <Text color="gray.600">
                            {selectedPrompt !== "all"
